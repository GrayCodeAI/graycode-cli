...
Hawk treats the agent runtime as a product boundary around provider output.

## Tool-Call Path

1. Eyrie normalizes provider protocol responses.
2. Hawk validates and resolves tool metadata.
...
Eyrie's verification package; Hawk owns host-level UX, permissions, persistence,
