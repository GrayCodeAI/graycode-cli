# Graycode Execution Graph

## Status

The first read-only execution-graph projection is implemented. It does not
replace Graycode's scheduler or move runtime ownership into a graph database.
...
eagle/graph
        ...
        |
        +--> privacy-normalized, explicit Graycode Cloud sync
```
...
|---|---|---|
| Graycode sessions and messages | `internal/session` | Read-only projection |
...
| Retrieved memory context | Harrier | Exact metadata-only retrieved subgraph is journaled and linked to the Graycode session |
| Retrieved code context | Graycode code index in Harrier storage | Metadata-only code-chunk knowledge nodes are journaled |
...
Graycode's structured task store now validates its blocking dependency graph before
...
default agent/tool loop remains authoritative, but `graycode mission --from-tasks`
...
- `graycode/session/<id>` — persisted Graycode session;
- `graycode/task-request/<session>/<n>` — user task request, content represented by
  SHA-256 only;
- `graycode/task/<id>` — structured task;
- `graycode/runtime-task/<id>` — background agent, shell, or monitor task;
- `graycode/tool-call/<session>/<tool-use-id>` — tool invocation metadata;
- `graycode/policy/<id>` — permission verdict;
- `graycode/verification/<id>` — neutral verification result;
- `harrier/memory/<id>` — retrieved memory knowledge;
- `graycode/code-chunk/<digest>` — retrieved code-index knowledge;
...
- `references` — related tasks and Graycode-to-Swift checkpoint linkage;
...
Graycode writes runtime observations to a per-session append-only JSONL journal.
The journal is stored with mode `0600` under Graycode's state directory and contains
...
portable `graycode.graph/v1` envelope, but it is mission-scoped rather than
...
`SWIFT_TAG_GRAYCODE_SESSION_ID=<graycode-session-id>` at its actual session-start
...
Graycode export, and incomplete checkpoint enumeration produces a session link
...
returns the typed `graycode.graph/v1` envelope. The handler owns transport concerns
only; the Graycode composition root injects the existing graph builder, so CLI,
...
After connecting a project with `graycode cloud login` or `graycode cloud connect`, a
...
Graycode does not upload prompt bodies, response bodies, tool arguments, tool
results, or other large artifacts.

Mission-scoped graph artifacts can use the same explicit path without being
pretended to be a Graycode session:

```bash
graycode graph export --mission-dir <mission-directory>
graycode cloud graph sync --mission-dir <mission-directory>
```

The CLI reads only `mission-graph.json`, validates its `graycode.graph/v1` schema
...
- Graycode code-index chunks selected through Harrier-backed code search.
...
graph-driven `graycode mission --from-tasks` path additionally emits operations
...
quality topology and Graycode's neutral aggregate verification observation. The
...
pre-turn guard. Existing Graycode cost accounting and limits remain authoritative
...
`graph correlation --graycode-session <id>` surface through the permitted
...
- returns the authoritative Swift session and checkpoint IDs for that Graycode ID;
...
Graycode now consumes this surface through `cli.NewRootCmd()`, validates the schema
and echoed Graycode identity, bounds the response, and emits:

- Graycode session `references` Swift session;
- Graycode session `references` each authoritative Swift checkpoint;
