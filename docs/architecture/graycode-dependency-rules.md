...
[`graycode/ecosystem.yaml`](../../ecosystem.yaml). `owl/ecosystem.json` is a
...
It remains upstream-only and must not import Graycode, Eagle, or an engine.
...
### 1. Graycode is the orchestrator

Only Graycode coordinates the support engines and owns the user-facing CLI,
...
Graycode currently imports selected engine-owned graph/projection packages for
...
`graycode-platform` is outside the Graycode Go module graph. Graycode may call the
