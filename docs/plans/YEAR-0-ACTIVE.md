# Year 0 Active Track (Grok → Graycode)
...
| Map capabilities to graycode-eco repos | Collapse engines into graycode monorepo |
| Wire existing modes/budgets first | Rebuild graycode-router/harrier/shrike as Grok clones |
...
- [x] Folder trust gates project hooks / plugins (`.graycode/plugins`, `.graycode/hooks`; MCP/LSP follow same AllowLoadPath)
...
- [x] User-guide docs `01`–`24` under `graycode/docs/user-guide/` (completed July 2026)
...
| Env | Default | Purpose |
|-----|---------|---------|
| `GRAYCODE_Y0_SPAWN_V2` | `1` once PACK-02 ships; `0` during dual path | Typed SpawnRequest path |
| `GRAYCODE_Y0_FOLDER_TRUST` | `1` recommended after PACK-03 | Gate project automation |
| `GRAYCODE_Y0_MARKETPLACE` | `0` until PACK-05 + trust | Marketplace install path |
