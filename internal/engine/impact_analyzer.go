	...
	_ = filepath.Walk(projectDir, func(path string, info os.FileInfo, err error) error {
		...
		for _, imp := range f.Imports {
			...
			if strings.HasPrefix(importPath, modulePath) {
				...
				graph[importPath] = impactAppendUnique(graph[importPath], thisPkg)
			}
		}
		return nil
	})
...
// impactAppendUnique appends s to slice only if not already present.
func impactAppendUnique(slice []string, s string) []string {
