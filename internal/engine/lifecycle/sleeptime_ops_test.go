...
func TestParseAndApplyMemoryOpsNoArray(t *testing.T) {
	if err := ParseAndApplyMemoryOps(&memory.YaadBridge{}, "no ops here"); !errors.Is(err, ErrNoMemoryOps) {
		t.Fatalf("expected ErrNoMemoryOps, got %v", err)
	}
}

func TestParseAndApplyMemoryOpsMalformedJSON(t *testing.T) {
	err := ParseAndApplyMemoryOps(&memory.YaadBridge{}, `prefix [{"op":"add","content":"x"} suffix`)
	if err == nil {
