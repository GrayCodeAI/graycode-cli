...
func (dg *DocGenerator) Generate() (*ProjectDoc, error) {
	...
	// Infer architecture from package structure
...
// findPackages recursively finds Go packages in the given directory.
func (dg *DocGenerator) findPackages(dir string, depth int) ([]PackageDoc, error) {
	...
	// Check if current directory has Go files
...
// parseGoPackage parses a Go package directory and extracts documentation.
...
// extractFunctions extracts function documentation from an AST file.
func (dg *DocGenerator) extractFunctions(file *ast.File, pkgDoc *PackageDoc) {
	for _, decl := range file.Decls {
		...
		// Skip methods (they are attached to types)
...
// extractTypes extracts type documentation from an AST file.
func (dg *DocGenerator) extractTypes(file *ast.File, pkgDoc *PackageDoc) {
	for _, decl := range file.Decls {
		...
		for _, spec := range genDecl.Specs {
			...
			// Find methods for this type across the file
...
// attachMethods finds methods associated with a type and attaches them.
func (dg *DocGenerator) attachMethods(file *ast.File, typeDoc *TypeDoc) {
	for _, decl := range file.Decls {
		...
		// Check if receiver matches this type
...
// buildMethodSignature builds a method signature for interface methods.
...
func (dg *DocGenerator) InferDescription(projectDir string) string {
	...
	// Try package doc comment from main package or first package found
...
// inferArchitecture infers project architecture from package structure.
...
// extractDescriptionFromREADME extracts the first paragraph after the title from a README.
func extractDescriptionFromREADME(content string) string {
	...
	for _, line := range lines {
		...
		if !pastTitle {
			...
			// If there's no # title, first non-empty line is description
			pastTitle = true
		}
		...
		if pastTitle {
			...
			// Skip badges and links-only lines
			if strings.HasPrefix(trimmed, "[![") || strings.HasPrefix(trimmed, "![") {
				continue
			}
			// Skip heading lines for next sections
			if strings.HasPrefix(trimmed, "## ") {
				break
			}
...
// extractDocComment extracts text from a comment group.
