...
func (cg *CodeGraph) createSchema() error {
	...
	END;
	`
	_, err := cg.db.ExecContext(context.Background(), schema)
	return err
}
...
func (cg *CodeGraph) IndexFile(filePath string) error {
	...
	tree, err := cg.parser.ParseCtx(context.Background(), nil, source)
	if err != nil {
		return err
	}
	...
	cg.mu.Lock()
	_, _ = cg.db.ExecContext(context.Background(), "DELETE FROM nodes WHERE file_path = ?", filePath)
	_, _ = cg.db.ExecContext(context.Background(), "DELETE FROM edges WHERE source IN (SELECT id FROM nodes WHERE file_path = ?)", filePath)
	cg.mu.Unlock()
...
func (cg *CodeGraph) ResolveRefs() error {
	...
	rows, err := cg.db.QueryContext(context.Background(), `SELECT id, from_node_id, reference_name, reference_kind, line, file_path, language FROM unresolved_refs`)
	if err != nil {
		return err
	}
	defer rows.Close() //nolint:errcheck // deferred Close on read-only rows is cleanup-only
	...
	_, _ = cg.db.ExecContext(context.Background(), `DELETE FROM unresolved_refs`)
...
// Standalone extraction/formatting helpers and the high-level read/traversal
// operations (Sync, Trace, Explore, Files, Status, searchByName, GetNode) live
// in codegraph_cgo_query.go.
