// Package tracebridge provides a Go library bridge that wraps trace's
// functionality for use by hawk, replacing the subprocess-based approach.
//
// Since trace and hawk are separate Go modules, this package uses
// interface-based decoupling. The bridge defines thin interfaces that
// trace types satisfy, avoiding a direct go.mod dependency.
package tracebridge
...
// TraceSessionManager defines the operations a trace session manager must
// support. In production this is satisfied by trace's session.StateStore
...
// TraceSession represents an individual capture session. It abstracts
// over trace's session.State without importing the trace module.
...
// SessionCapture wraps trace's session capture functionality for use as a
...
// or environments where trace is not available).
...
func (sc *SessionCapture) StartCapture(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("tracebridge: start capture: %w", err)
	}
...
func (sc *SessionCapture) StopCapture(ctx context.Context) (*CaptureResult, error) {
	...
	if err := sc.session.Close(ctx); err != nil {
		return nil, fmt.Errorf("tracebridge: stop capture: %w", err)
	}
...
// TRACE_TAG_* convention: the key is normalized (lowercased, hyphens
...
// (nil/noop) session manager; inject a real TraceSessionManager via
// NewSubprocessBridgeAdapterWithManager if trace is available.
...
// backed by a specific TraceSessionManager.
...
// Ready reports whether the underlying trace session manager is
...
// normalizeTagKey applies the same normalization as trace's session tag
// handling: strip TRACE_TAG_ prefix, lowercase, replace hyphens with
// underscores.
...
// noopSessionManager is a TraceSessionManager that does nothing. Used
// when no real trace manager is available (e.g., trace not installed).
type noopSessionManager struct{}

func (n *noopSessionManager) Start(_ context.Context, _, _ string) (TraceSession, error) {
	return &noopSession{}, nil
}

func (n *noopSessionManager) Load(_ context.Context, _ string) (TraceSession, error) {
	return &noopSession{}, nil
}

// noopSession is a TraceSession that records nothing.
