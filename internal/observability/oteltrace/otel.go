...
func DefaultTelemetryConfig() TelemetryConfig {
	...
	// Telemetry is opt-in: only the explicit HAWK_ENABLE_TELEMETRY=1
	...
	cfg.Enabled = os.Getenv("HAWK_ENABLE_TELEMETRY") == "1"

	if hdrs := os.Getenv("OTEL_EXPORTER_OTLP_HEADERS"); hdrs != "" {
		cfg.Headers = parseHeaders(hdrs)
	}

	if timeout := os.Getenv("HAWK_OTEL_SHUTDOWN_TIMEOUT_MS"); timeout != "" {
