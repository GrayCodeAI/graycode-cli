...
func TestDefaultTelemetryConfig(t *testing.T) {
	os.Setenv("HAWK_ENABLE_TELEMETRY", "1")
	defer os.Unsetenv("HAWK_ENABLE_TELEMETRY")

	cfg := DefaultTelemetryConfig()
	if !cfg.Enabled {
		t.Error("expected telemetry enabled when env var set")
	}
	if cfg.ServiceName != "hawk" {
		t.Errorf("expected service name hawk, got %s", cfg.ServiceName)
	}
...
func TestDefaultTelemetryConfig_Disabled(t *testing.T) {
	os.Unsetenv("HAWK_ENABLE_TELEMETRY")
