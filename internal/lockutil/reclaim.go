...
// keeps the window during which path does not exist as short as possible.
// RestoreLockFile's no-replace restore is a correctness-preserving fallback
// for when the fast path itself fails.
...
// failed for a reason that is not contention, or a live holder's lock could not
// be restored, so lockPath may be missing; callers must fail closed instead of
// re-acquiring. The sidelined file is removed on every restore failure.
