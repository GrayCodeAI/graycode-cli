...
// EyrieTool is Graycode's runtime tool definition DTO.
...
// DefaultContinuationConfig is Graycode's agent-loop continuation policy. Eyrie
...
// EyrieUsage tracks token usage for Graycode runtime responses and streams.
type EyrieUsage = llm.EyrieUsage

// ResolvedRoute is Graycode's view of the concrete provider/model route selected
// by the provider engine. Keeping this projection Graycode-owned prevents Eyrie's
// transport DTOs from leaking into product state and observability payloads.
type ResolvedRoute = llm.ResolvedRoute

// EyrieResponse is Graycode's runtime chat response DTO.
type EyrieResponse = llm.EyrieResponse

// EyrieStreamEvent is Graycode's runtime stream event DTO.
...
// EyrieMessage is Graycode's runtime conversation DTO.
