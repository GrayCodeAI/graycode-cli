...
func WriteReportToPath(path string, s *Snapshot) error {
	...
	if err != nil {
		return fmt.Errorf("create swift file: %w", err)
	}
	if _, err := f.WriteString(contents); err != nil {
		_ = f.Close()
		_ = os.Remove(path)
		return fmt.Errorf("write swift file: %w", err)
	}
	if err := f.Close(); err != nil {
		return fmt.Errorf("close swift file: %w", err)
	}
	return nil
}
...
// writeSwiftReportFile (0o600 mode, up to 8 collision attempts, random suffix).
...
func WriteReportFile(s *Snapshot) (string, error) {
	...
	for attempt := 0; attempt < 8; attempt++ {
		...
		if werr != nil {
			_ = os.Remove(path)
			return "", fmt.Errorf("write swift file: %w", werr)
		}
		if cerr != nil {
			return "", fmt.Errorf("close swift file: %w", cerr)
		}
		return path, nil
	}
	return "", fmt.Errorf("could not allocate a unique swift file name")
}
...
func isDarwin() bool {
	return os.Getenv("SWIFT_FORCE_DARWIN") == "1"
}
