...
func TestAttributionModesIndependent(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			modes:        &AttributionModes{CoAuthoredBy: "Hawk <hawk@hawk.ai>"},
			wantCoAuthor: true,
		},
		{
			name:          "author only",
		...
		},
		{
			...
			modes: &AttributionModes{
				...
				CoAuthoredBy: "Hawk <hawk@hawk.ai>",
			},
			wantCoAuthor:     true,
			wantAuthorEnv:    true,
			wantCommitterEnv: true,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			hasCo := strings.Contains(msg, "Co-authored-by: Hawk <hawk@hawk.ai>")
...
func TestCommitStagedWithAttribution(t *testing.T) {
	...
	body := gitCommitBody(t)
	if !strings.Contains(body, "Co-authored-by: Hawk <hawk@hawk.ai>") {
