...
check_layer() {
  ...
  local forbidden=$2
  local matches
  matches=$(git grep -n -E "github.com/GrayCodeAI/hawk/(${forbidden})([\"/])" -- "${directory}/**/*.go" \
...
if ((failed)); then
  echo "Hawk internal layer boundary check failed" >&2
  exit 1
fi
echo "Hawk internal layer boundary check passed"
