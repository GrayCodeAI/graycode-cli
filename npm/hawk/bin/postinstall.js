#!/usr/bin/env node
// Runs once after npm install/update. Reads the Hawk binary from the matching
// per-platform optional dependency (@graycodeai/hawk-<platform>-<arch>) and
// installs it to ~/.hawk/bin/ using versioned filenames:
//
//   Unix:    hawk-<version>  +  hawk  (symlink)
//   Windows: hawk-<version>.exe  +  hawk.exe  (copy)
//
...
// the same versioned-at-~/.hawk/bin/+symlink convention. Install either path;
// they converge on the same layout.
//
// Suppress entirely with HAWK_SKIP_POSTINSTALL=1 (useful for CI / controlled
...
// Canonical install location mirrors install.sh (HAWK_HOME default ~/.hawk).
const HAWK_HOME = process.env.HAWK_HOME || path.join(os.homedir(), '.hawk');
const CANONICAL_DIR = path.join(HAWK_HOME, 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@graycodeai/hawk: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@graycodeai/hawk-${key}`;
    try {
...
if (!version) {
    console.error('@graycodeai/hawk: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@graycodeai/hawk: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@graycodeai/hawk: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running hawk processes and try again.');
                return false;
            }
        }
    } else {
...
if (process.env.HAWK_SKIP_POSTINSTALL === '1') {
    console.log('@graycodeai/hawk: HAWK_SKIP_POSTINSTALL=1 set; skipping binary install.');
    process.exit(0);
}

const platformDir = resolvePlatformPackageDir();
if (!platformDir) {
    console.error(`@graycodeai/hawk: platform package @graycodeai/hawk-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @graycodeai/hawk');
    process.exit(0);
}

installBinary('hawk', platformDir, `hawk${EXE}`);
cleanupOldVersions('hawk');

console.log('');
if (process.env.HAWK_SKIP_HINT !== '1') {
    console.log(`Add ${CANONICAL_DIR} to your PATH if it is not already, e.g.`);
    console.log(`  export PATH="$PATH:${CANONICAL_DIR}"`);
    console.log('');
    console.log('Restart any running hawk sessions to pick up the new binary — the old');
