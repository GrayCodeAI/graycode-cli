...
//   1. Brotli-compresses the built Graycode binary into
//      `../graycode-<platform>-<arch>/bin/<bin>.br`
...
// (`@graycodeai/graycode`) lists all six as `optionalDependencies` pinned to
...
// Graycode's binary is the plain Go binary named `graycode` (or `graycode.exe`), produced
// by GoReleaser (`.goreleaser.yml`, main `./cmd/graycode`, project_name `graycode`).
const fs = require('fs');
...
const NOTICES_SOURCE = process.env.GRAYCODE_THIRD_PARTY_NOTICES
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `graycode-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] graycode-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    ...
    // with binary `graycode` (Unix) / `graycode.exe` (Windows). The build ID is `graycode`,
    // so the dir is `dist/graycode_<os>_<arch>_v1`. The `[vN]` arch version suffix
    // is GoReleaser's GOAMD64 default (v1 for amd64); if your config changes
    // this, set the GRAYCODE_* env vars instead of relying on the defaults.
    const goosGoarch = {
    ...
    function gorel(platform, arch) {
        ...
        const bin = platform === 'win32' ? 'graycode.exe' : 'graycode';
        const dirName = `graycode_${os}_${ga}${ga === 'arm64' ? '' : '_v1'}`;
        return path.join(distRoot, dirName, bin);
    }

    // Note: graycode's `.goreleaser.yml` currently ignores windows/arm64 in builds,
    // so graycode-win32-arm64 has no default GoReleaser artifact yet. The target is
    // retained so that when the build matrix adds it (or CI supplies the
    // binary via GRAYCODE_WIN32_ARM64), packaging works without script changes.
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'graycode',
            envVar: 'GRAYCODE_DARWIN_ARM64',
            defaultSource: gorel('darwin', 'arm64'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'graycode',
            envVar: 'GRAYCODE_DARWIN_X64',
            defaultSource: gorel('darwin', 'x64'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'graycode',
            envVar: 'GRAYCODE_LINUX_X64',
            defaultSource: gorel('linux', 'x64'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'graycode',
            envVar: 'GRAYCODE_LINUX_ARM64',
            defaultSource: gorel('linux', 'arm64'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'graycode.exe',
            envVar: 'GRAYCODE_WIN32_X64',
            defaultSource: gorel('win32', 'x64'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'graycode.exe',
            envVar: 'GRAYCODE_WIN32_ARM64',
