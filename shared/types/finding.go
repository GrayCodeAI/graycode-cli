...
// Sort order: severity descending, then confidence descending.
...
func (s FindingSlice) Less(i, j int) bool {
	if s[i].Severity != s[j].Severity {
		return s[i].Severity > s[j].Severity // higher severity first
	}
	return s[i].Confidence > s[j].Confidence // higher confidence first
}
