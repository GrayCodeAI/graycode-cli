...
func (ch *ContextualHelp) registerAllEntries() {
	entries := []*HelpEntry{
		...
		},
		{
			...
			Examples: []string{"/config              — open config panel", "/config model        — change model", "/config key          — set API key"},
		...
		// ─── Error Explanations ──────────────────────────────────────
		{
			...
			Detail:   "Your API key is not configured or has expired. Set it via /config key or the HAWK_API_KEY environment variable.",
			Examples: []string{"/config key          — set API key interactively", "export HAWK_API_KEY=sk-...", "hawk --key sk-...    — pass key as flag"},
		...
		},
		{
			...
			Detail:   "Configure your API key for authentication. Can be set via config, environment variable, or command flag.",
			Examples: []string{"/config key sk-...   — set key directly", "export HAWK_API_KEY=sk-...", "hawk --key sk-..."},
