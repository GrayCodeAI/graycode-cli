...
// "mouse": false). Priority: HAWK_MOUSE env → runtime override → settings → default on.
...
func (m *chatModel) handleMouseCommand(parts []string) {
	...
	if envOverridesMouse() {
		m.messages = append(m.messages, displayMsg{
			role: "system",
			content: "Mouse is controlled by HAWK_MOUSE env in this session. " +
	...
	case "on", "true", "1", "enable":
		...
		_ = hawkconfig.SetGlobalSetting("tui_mouse", "true")
	...
	case "toggle":
		...
		_ = hawkconfig.SetGlobalSetting("tui_mouse", val)
