...
// CompletionGenerator generates shell completion scripts for graycode.
...
// NewCompletionGenerator creates a CompletionGenerator pre-populated with graycode's
...
func (g *CompletionGenerator) populateProviders() {
	g.Providers = graycodeconfig.AllSetupGateways()
}
...
// GenerateBash returns a complete bash completion script for graycode.
...
// GenerateZsh returns a complete zsh completion script for graycode.
...
// GenerateFish returns a complete fish shell completion script for graycode.
func (g *CompletionGenerator) GenerateFish() string {
	...
	for _, cmd := range g.Commands {
		...
		if len(cmd.Flags) > 0 {
			...
			for _, f := range cmd.Flags {
				if f.Short != "" {
					b.WriteString(fmt.Sprintf("complete -c graycode -n '__fish_seen_subcommand_from %s' -l '%s' -s '%s' -d '%s'",
						cmd.Name, f.Name, f.Short, escapeFish(f.Description)))
				} else {
					b.WriteString(fmt.Sprintf("complete -c graycode -n '__fish_seen_subcommand_from %s' -l '%s' -d '%s'",
	...
	for _, f := range g.Flags {
		if f.Short != "" {
			b.WriteString(fmt.Sprintf("complete -c graycode -l '%s' -s '%s' -d '%s'",
				f.Name, f.Short, escapeFish(f.Description)))
		} else {
			b.WriteString(fmt.Sprintf("complete -c graycode -l '%s' -d '%s'",
	...
	for _, sc := range g.SlashCommands {
		b.WriteString(fmt.Sprintf("complete -c graycode -a '%s' -d 'Slash command'\n", sc))
	}
...
func bashInstallPath() string {
	...
	if runtime.GOOS == "darwin" {
		for _, prefix := range []string{"/opt/homebrew", "/usr/local"} {
			...
			if info, err := os.Stat(brewDir); err == nil && info.IsDir() {
				return filepath.Join(brewDir, "graycode")
			}
		}
	}
	...
	if info, err := os.Stat(sysDir); err == nil && info.IsDir() {
		return filepath.Join(sysDir, "graycode")
	}
	// Final fallback: user-local
	return filepath.Join(home, ".local", "share", "bash-completion", "completions", "graycode")
}
		...
		for _, p := range parts {
			if p != "" {
				if info, err := os.Stat(p); err == nil && info.IsDir() { // #nosec G703 -- shell completion only probes the user-selected directory
					return filepath.Join(p, "_graycode")
				}
			}
		}
		// Use first entry even if it doesn't exist yet
		if parts[0] != "" {
			return filepath.Join(parts[0], "_graycode")
		}
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "/usr/local/share/zsh/site-functions/_graycode"
	}
	return filepath.Join(home, ".zsh", "completions", "_graycode")
}

func fishInstallPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(os.TempDir(), "graycode.fish")
	}
	return filepath.Join(home, ".config", "fish", "completions", "graycode.fish")
}
