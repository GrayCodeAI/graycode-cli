...
var cloudConnectCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		cmd.Println(auditTint("Hawk Cloud connected. Usage synchronization is opt-in and fail-open.", doneGreen))
		return nil
	},
}
...
var cloudLoginCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		cmd.Printf("%s\n", auditTint("Open ", textPrimary)+auditTint(start.VerificationURI, infoSky)+auditTint(" and enter code ", textPrimary)+auditTint(start.UserCode, hawkColor))
		...
		for {
			...
			case "approved":
				...
				prog.CompleteStep(0)
				prog.Done()
				cmd.Println(auditTint("Hawk Cloud connected for project ", doneGreen) + auditTint(poll.ProjectID, textPrimary) + auditTint(".", doneGreen))
				return nil
			case "expired":
...
var cloudStatusCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		if err != nil || !client.Enabled() {
			cmd.Println(auditTint("Hawk Cloud is not connected.", textMuted))
			return nil
		}
		cmd.Println(auditTint("Hawk Cloud connected: ", doneGreen) + auditTint(cfg.Endpoint, textPrimary) + auditTint(fmt.Sprintf(" (device %s, project %s)", cfg.DeviceID, cfg.ProjectID), textMuted))
		return nil
	},
}
...
var cloudContextCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		cmd.Println(auditTint("Repository context queued for Hawk Cloud.", doneGreen))
		return nil
	},
}
