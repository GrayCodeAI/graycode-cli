...
var planCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		cmd.Println(auditTint("--- System ---", hawkColor))
		cmd.Println(prompt.System)
		cmd.Println()
		cmd.Println(auditTint("--- User ---", hawkColor))
		cmd.Println(prompt.User)
		cmd.Println()
		cmd.Println(auditTint("Once you have the LLM response, save it with an explicit output path or import it into Hawk plans.", textMuted))
		return nil
	},
}
		...
		if err != nil {
			if os.IsNotExist(err) {
				if planJSON {
					...
					cmd.Println(auditTint("No plans found. Create one with: hawk plan create <description>", textMuted))
				}
				return nil
			}
