...
// ErrorContext provides context-aware error messages by matching error strings
// against known patterns and returning rich contextual help.
...
// NewErrorContext creates an ErrorContext pre-loaded with 30+ common error patterns
// covering Go, Python, JS/TS, Git, system errors, and hawk-specific errors.
...
// Enrich takes a raw error string and returns an EnrichedError with contextual
// help if the error matches any known pattern. Returns nil if no pattern matches.
// When multiple patterns match, the longest matching pattern string is preferred
...
// FormatError formats an EnrichedError into a human-readable multi-line string
// with visual separators and structured sections.
...
// AddPattern registers a new error pattern with its associated help information.
// Returns an error if the pattern string fails to compile as a regular expression.
...
// IsRecoverable returns true if the given error is classified as recoverable,
// meaning the operation can potentially succeed if retried or modified.
...
// SuggestFix returns a quick one-liner suggestion for fixing the given error.
...
// classifySeverity determines the severity level of an error based on keywords.
...
// classifyRecoverable determines whether an error is likely recoverable.
...
// sanitizePatternKey creates a map key from a regex pattern string.
