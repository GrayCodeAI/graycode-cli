...
func ApplyFix(action CodeAction, content string) (string, error) {
	...
	// Find the rule to get the pattern
	...
	// Fallback: replace the entire line with the fix preserving indentation
...
func detectLanguageFromPath(path string) string {
	...
	case strings.HasSuffix(lower, ".js"), strings.HasSuffix(lower, ".jsx"):
		return "typescript" // JS rules overlap significantly
...
func isMultilinePattern(rule ActionRule) bool {
	// Patterns that use (?s) or contain \n or brace matching are multiline
...
func generateFix(rule ActionRule, matched string) string {
	...
	// Simple template without Go template syntax
...
func computeConfidence(rule ActionRule) float64 {
	// Higher priority rules tend to have higher confidence
...
func extractAddedLines(diff string) diffLines {
	...
	_ = currentLine // used for line tracking during parse
